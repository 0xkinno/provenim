/**
 * Isomorphic, zero-dependency synchronous SHA-256 implementation.
 * Works identically in Node.js, browsers, webviews, and service workers.
 */
...
export function sha256Sync(ascii: string): string {
  ...
  // First 64 round constants (fractional parts of cube roots of first 64 primes 2..311):
  const k = [
  ...
  words.push(Math.floor(utf8BitLength / maxWord));
  words.push(utf8BitLength & 0xffffffff);
